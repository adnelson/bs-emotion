...
module Declarations = {
  ...
  let toDict: list<declaration> => Js.Dict.t<_> = declarations =>
    declarations->unpack->List.flatten->Js.Dict.fromList
}

module Selector = {
  external pack: list<(string, Js.Dict.t<_>)> => declaration = "%identity"
  external unpack: declaration => list<(string, Js.Dict.t<_>)> = "%identity"
}

/* ===== 🗂 CSS Properties ===== */

let p = (prop, value) => list{(prop, value)}->Declaration.pack

let important = (v: declaration) =>
  switch v->Declaration.unpack {
  | list{(prop, value)} => list{(prop, value ++ " !important")}->Declaration.pack
...
let margin3 = (t, h, b) => p("margin", LengthPercentageAuto.toString3(t, h, b))
let margin4 = (t, r, b, l) => p("margin", LengthPercentageAuto.toString4(t, r, b, l))
...
let padding3 = (t, h, b) => p("padding", LengthPercentage.toString3(t, h, b))
let padding4 = (t, r, b, l) => p("padding", LengthPercentage.toString4(t, r, b, l))
...
let border = (width, style, color) => p("border", Border.toString(width, style, color))
...
let borderLeft = (width, style, color) => p("borderLeft", Border.toString(width, style, color))
...
let borderRight = (width, style, color) => p("borderRight", Border.toString(width, style, color))
...
let borderTop = (width, style, color) => p("borderTop", Border.toString(width, style, color))
...
let borderBottom = (width, style, color) => p("borderBottom", Border.toString(width, style, color))
...
let borderTopLeftRadius = x => p("borderTopLeftRadius", x->LengthPercentage.toString)
let borderTopRightRadius = x => p("borderTopRightRadius", x->LengthPercentage.toString)
let borderBottomLeftRadius = x => p("borderBottomLeftRadius", x->LengthPercentage.toString)
let borderBottomRightRadius = x => p("borderBottomRightRadius", x->LengthPercentage.toString)
...
let backgroundAttachment = x => p("backgroundAttachment", x->BackgroundAttachment.toString)
let backgroundAttachments = x =>
  p("backgroundAttachment", x->List.map(BackgroundAttachment.toString)->Helpers.joinWith(", "))
let backgroundBlendMode = x => p("backgroundBlendMode", x->BackgroundBlendMode.toString)
let backgroundBlendModes = x =>
  p("backgroundBlendMode", x->List.map(BackgroundBlendMode.toString)->Helpers.joinWith(", "))
...
let backgroundImages = x =>
  p("backgroundImage", x->List.map(BackgroundImage.toString)->Helpers.joinWith(", "))
let backgroundOrigin = x => p("backgroundOrigin", x->BackgroundBox.toString)
let backgroundPosition = x => p("backgroundPosition", x->BackgroundPosition.toString)
let backgroundPositions = x =>
  p("backgroundPosition", x->List.map(BackgroundPosition.toString)->Helpers.joinWith(", "))
let backgroundRepeat = x => p("backgroundRepeat", x->BackgroundRepeat.toString)
let backgroundRepeat2 = (x, y) => p("backgroundRepeat", BackgroundRepeat.toString2(x, y))
let backgroundSize = x => p("backgroundSize", x->BackgroundSize.toString)
let backgroundSizes = x =>
  p("backgroundSize", x->List.map(BackgroundSize.toString)->Helpers.joinWith(", "))
...
let fontWeight = (x: int) => p("fontWeight", x->Js.Int.toString)
...
let src = (srcs: list<(FontSrc.src, option<FontSrc.format>)>) =>
  p(
    "src",
    srcs->List.map(((src, format)) => FontSrc.toString(~format?, src))->Helpers.joinWith(", "),
  )
...
let textDecorationStyle = x => p("textDecorationStyle", x->TextDecorationStyle.toString)
...
let textShadows = xs =>
  p(
    "textShadow",
    xs
    ->List.mapU((. (x, y, blur, color)) => TextShadow.toString(~x, ~y, ~blur, color))
    ->Helpers.joinWith(", "),
  )
...
let listStylePosition = x => p("listStylePosition", x->ListStylePosition.toString)
let listStyleImage = x => p("listStyleImage", x->ListStyleImage.toString)

let opacity = (x: float) => p("opacity", x->Js.Float.toString)

let cursor = x => p("cursor", x->Cursor.toString)

let pointerEvents = x => p("pointerEvents", x->PointerEvents.toString)

let outline = (size, style, color) => p("outline", Outline.toString(size, style, color))
...
let transition = (property: string, duration, timingFunction, delay) =>
  p("transition", Transition.toString(~property, ~duration, ~delay, ~timingFunction))
...
let transitionTimingFunction = x => p("transitionTimingFunction", x->TimingFunction.toString)

let transform = x => p("transform", x->Transform.toString)
let transforms = xs => p("transform", xs->List.map(Transform.toString)->Helpers.joinWith(" "))
let transformOrigin = (x, y) => p("transformOrigin", LengthPercentage.toString2(x, y))
let transformOrigin3d = (x, y, z) => p("transformOrigin", LengthPercentage.toString3(x, y, z))
let transformStyle = x => p("transformStyle", x->TransformStyle.toString)

let perspective = x => p("perspective", x->Perspective.toString)

let perspectiveOrigin = (x, y) => p("perspectiveOrigin", LengthPercentage.toString2(x, y))

let fill = x => p("fill", x->Color.toString)
let fillOpacity = (x: float) => p("fillOpacity", x->Js.Float.toString)
...
let strokeOpacity = (x: float) => p("strokeOpacity", x->Js.Float.toString)
let strokeMiterlimit = (x: float) => p("strokeMiterlimit", x->Js.Float.toString)
...
let stopOpacity = (x: float) => p("stopOpacity", x->Js.Float.toString)
...
let animationDelays = x => p("animationDelay", x->List.map(Timing.toString)->Helpers.joinWith(", "))
let animationDirection = x => p("animationDirection", x->AnimationDirection.toString)
let animationDirections = x =>
  p("animationDirection", x->List.map(AnimationDirection.toString)->Helpers.joinWith(", "))
let animationDuration = x => p("animationDuration", x->Timing.toString)
let animationDurations = x =>
  p("animationDuration", x->List.map(Timing.toString)->Helpers.joinWith(", "))
let animationFillMode = x => p("animationFillMode", x->AnimationFillMode.toString)
let animationFillModes = x =>
  p("animationFillMode", x->List.map(AnimationFillMode.toString)->Helpers.joinWith(", "))
let animationIterationCount = x => p("animationIterationCount", x->AnimationIterationCount.toString)
...
let animationNames = (x: list<string>) => p("animationName", x->Helpers.joinWith(", "))
let animationPlayState = x => p("animationPlayState", x->AnimationPlayState.toString)
let animationPlayStates = x =>
  p("animationPlayState", x->List.map(AnimationPlayState.toString)->Helpers.joinWith(", "))
let animationTimingFunction = x => p("animationTimingFunction", x->TimingFunction.toString)
let animationTimingFunctions = x =>
  p("animationTimingFunction", x->List.map(TimingFunction.toString)->Helpers.joinWith(", "))

let filter = x => p("filter", x->Filter.toString)
let filters = x => p("filter", x->List.map(Filter.toString)->Helpers.joinWith(" "))

let appearance = x => p("appearance", x->Appearance.toString)

let flex = x => p("flex", x->Flex.Flex.toString)
let flexGrow = (x: float) => p("flexGrow", x->Js.Float.toString)
let flexShrink = (x: float) => p("flexShrink", x->Js.Float.toString)
...
let flexFlow = (direction, wrap) => p("flex-flow", Flex.Flow.toString(direction, wrap))
let order = (x: int) => p("order", x->Js.Int.toString)

let gridTemplateRows = x => p("gridTemplateRows", x->Grid.Template.toString)
let gridTemplateColumns = x => p("gridTemplateColumns", x->Grid.Template.toString)
...
let gridTemplateAreas = x => p("gridTemplateAreas", x->Grid.TemplateAreas.toString)
...
let zIndex = (x: int) => p("zIndex", x->Js.Int.toString)

let content = (x: string) => p("content", x->Js.Json.string->Js.Json.stringify)
...
let select = (selector, declarations) =>
  list{(selector, declarations->Declarations.toDict)}->Selector.pack

let active = decs => select(":active", decs)
let after = decs => select("::after", decs)
let before = decs => select("::before", decs)
let checked = decs => select(":checked", decs)
let disabled = decs => select(":disabled", decs)
let firstChild = decs => select(":first-child", decs)
let firstOfType = decs => select(":first-of-type", decs)
let focus = decs => select(":focus", decs)
let hover = decs => select(":hover", decs)
let lastChild = decs => select(":last-child", decs)
let lastOfType = decs => select(":last-of-type", decs)
let link = decs => select(":link", decs)
let readOnly = decs => select(":read-only", decs)
let required = decs => select(":required", decs)
let visited = decs => select(":visited", decs)
let enabled = decs => select(":enabled", decs)
let noContent = decs => select(":empty", decs)
let default = decs => select(":default", decs)
let anyLink = decs => select(":any-link", decs)
let onlyChild = decs => select(":only-child", decs)
let onlyOfType = decs => select(":only-of-type", decs)
let optional = decs => select(":optional", decs)
let invalid = decs => select(":invalid", decs)
let outOfRange = decs => select(":out-of-range", decs)
let target = decs => select(":target", decs)
let firstLine = decs => select("::first-line", decs)
let firstLetter = decs => select("::first-letter", decs)
let selection = decs => select("::selection", decs)
let placeholder = decs => select("::placeholder", decs)

/* refmt issue, should be just `not` */
let not__ = (selector: string, declarations) => select(`:not(${selector})`, declarations)

let nthChild = (selector: string, declarations) => select(`:nth-child(${selector})`, declarations)
let nthLastChild = (selector: string, declarations) =>
  select(`:nth-last-child(${selector})`, declarations)
let nthOfType = (selector: string, declarations) => select(`:nth-of-type(${selector})`, declarations)
let nthLastOfType = (selector: string, declarations) =>
  select(`:nth-last-of-type(${selector})`, declarations)

/* ===== 📋 CSS @-rules ===== */

let media = (query: string, declarations) => select(`@media ${query}`, declarations)

let supports = (query: string, declarations) => select(`@supports ${query}`, declarations)
...
let linearGradient = (a: Angle.t, s: Gradient.Stops.t) => #linearGradient(a, s)
let repeatingLinearGradient = (a: Angle.t, s: Gradient.Stops.t) => #repeatingLinearGradient(a, s)
let radialGradient = (s: Gradient.Stops.t) => #radialGradient(s)
let repeatingRadialGradient = (s: Gradient.Stops.t) => #repeatingRadialGradient(s)
...
let cubicBezier = (a: float, b: float, c: float, d: float) => #cubicBezier(a, b, c, d)

let translate = (x: LengthPercentage.t, y: LengthPercentage.t) => #translate(x, y)
let translate3d = (x: LengthPercentage.t, y: LengthPercentage.t, z: LengthPercentage.t) =>
  #translate3d(x, y, z)
...
let rotate3d = (x: float, y: float, z: float, a: Angle.t) => #rotate3d(x, y, z, a)
...
let minmax = (min: Grid.MinMax.min, max: Grid.MinMax.max) => #minmax(min, max)
let minContent = #minContent
let maxContent = #maxContent
let fitContent = (x: LengthPercentage.t) => #fitContent(x)
let repeat = (value: Grid.Repeat.value, trackList: list<Grid.Repeat.trackList>) =>
...
let keyframes = (frames: list<(int, list<declaration>)>) =>
  frames
  ->List.reduce(Js.Dict.empty(), (acc, item) => {
    let stop = item->fst
    acc->Js.Dict.set(`${stop->Js.Int.toString}%`, item->snd->Declarations.toDict)
    acc
  })
  ->makeKeyframes
